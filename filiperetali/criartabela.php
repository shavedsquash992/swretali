...
try {$conexao = new PDO("mysql:host=$host;dbname=$dbname",$user,$pass);} catch (PDOException $th) {}
if($conexao){
    $texto="CREATE TABLE IF NOT EXISTS ".$_POST['tabela']." (
     ...
     try {        
        $acao->execute();
        echo "Tabela ".$_POST['banco']." criada com sucesso.";} catch (PDO $th) {echo $th.getMessage();}}
